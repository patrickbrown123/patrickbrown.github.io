...
-- Purpose: Data importing, cleaning, transformation & exploration
-- ============================================
...
FROM 'C:\Users\Patrick\portfolio_clean\superstore_data_raw.csv'
WITH (
    FIELDTERMINATOR = '\t',
    ROWTERMINATOR = '\n',
    FIRSTROW = 2
...
-- 3.4 Standardise categorical values
...
ALTER TABLE SuperstoreSales ADD Outlier_Flag BIT DEFAULT 0; --RUN THIS LINE BY ITSELF
...
ALTER TABLE SuperstoreSales ADD CustomerSegment VARCHAR(50); --RUN THIS LINE BY ITSELF
...
-- 4.2 First order date & number of months since first order
ALTER TABLE SuperstoreSales ADD MinOrderDate DATE; --RUN THIS LINE BY ITSELF
ALTER TABLE SuperstoreSales ADD OrderRelativeMonth INT; --RUN THIS LINE BY ITSELF
...
-- 4.3 Create view for PowerBI
CREATE VIEW superstore_data_cleaned AS
SELECT * FROM SuperstoreSales;
